...
pub struct Projectiton<'a>(pub(crate) &'a SelectItem);
...
/// 把sqlParser解析出来的statement 转换成我们需要的数据结构
impl<'a> TryFrom<&'a Statement> for Sql<'a> {
    type Error = anyhow::Error;
    fn try_from(value: &'a Statement) -> std::prelude::v1::Result<Self, Self::Error> {
        match value {
            Statement::Query(q) => {
                ...
                for p in projection {
                    let expr = Projectiton(p).try_into()?;
                    selection.push(expr);
                }
...
/// 把sqlParser的Expr转换成DataFrame的Expr
...
/// 把sqlParser的BinaryOperator转换成DataFrame的Operator
...
impl<'a> TryFrom<Projectiton<'a>> for Expr {
    type Error = anyhow::Error;
    fn try_from(value: Projectiton<'a>) -> Result<Self, Self::Error> {
        match value.0 {
